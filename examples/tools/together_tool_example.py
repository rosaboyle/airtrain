#!/usr/bin/env python3
"""
Example of using AirTrain's tool registry system with Together AI.
This script demonstrates how to use both stateful and stateless tools
with Together AI's API.
"""
...
# Import Together AI integration
from airtrain.integrations.together import (
    TogetherAIChatSkill,
    TogetherAIInput,
...
def run_with_together():
    """Run an example using Together AI with tool calls."""
    print("\n=== Demonstrating Tools with Together AI ===")
    ...
    # Initialize Together AI skill
    together_skill = TogetherAIChatSkill()
    
    # Prepare input with tool definitions
    input_data = TogetherAIInput(
        model="Qwen/Qwen2.5-72B-Instruct-Turbo",  # Model with function calling
    ...
    try:
        # Process the request
        result = together_skill.process(input_data)
        ...
        if tool_calls:
            ...
            for i, tool_call in enumerate(tool_calls):
                ...
                # followup_messages.append(result.response)
                ...
                followup_input = TogetherAIInput(
                    user_input="",
                ...
                followup_result = together_skill.process(followup_input)
                print("\nFinal response:")
                print(followup_result.response)
    
    except Exception as e:
        print(f"Error running Together AI example: {str(e)}")


def main():
    """Main function to demonstrate Together AI tool usage."""
