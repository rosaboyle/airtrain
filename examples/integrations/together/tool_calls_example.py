...
# NOTE: This example requires a valid TOGETHER_API_KEY environment variable
...
def run_conversation():
    ...
    system_prompt = (
        ...
        "For example, if asked about weather in London and Paris, call get_weather "
        "twice, once for each city. Always use the tools to get information."
    )
    ...
    while turn_count < max_turns:
        ...
        input_data = TogetherAIInput(
            ...
            tool_choice="auto",
            temperature=0.2,
            max_tokens=4096,  # Safe value for most models
        )
        ...
        if result.tool_calls:
            ...
            # Add the assistant message with the thinking content
            # Important: Airtrain expects strings in conversation_history
            assistant_thinking = "Let me check that information for you."
            ...
            follow_up = "Can you tell me about the weather and travel options?"
            conversation_history.append({
                "role": "user",
                "content": follow_up
            })
            
            # Create a new response that includes the tool results in a human-readable format
            tools_summary = ""
            for resp in tool_responses:
                tools_summary += f"- {resp['result']}\n"
            
            # Add assistant response with the summarized tool results
            assistant_response = f"Here's what I found:\n\n{tools_summary}"
            conversation_history.append({
                "role": "assistant",
            ...
            # For the next turn, we use the original conversation history plus the last exchange
            if turn_count == max_turns - 1:
                # On the last turn, just say thank you
                conversation_history.append({
                    "role": "user", 
                    "content": "Thank you for all the information!"
                })
                
                # Create final response input - important to avoid tool_calls here
                final_input = TogetherAIInput(
                    user_input="Thank you for all the information!",
                    system_prompt=system_prompt,
                    conversation_history=conversation_history,
                    model="Qwen/Qwen2.5-72B-Instruct-Turbo", 
                    temperature=input_data.temperature,
                    max_tokens=1024,  # Reduced for final response
                )
